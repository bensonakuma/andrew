$(document).ready(function () {
    ...
    //Open popup after leaving page
    var waitPopup = document.getElementById('wait-popup'); 
    var leavingCount = 0;

    if (waitPopup) {
        if (window.innerWidth <= 992 && waitPopupGetCookie('wait_popup') == null) {
            setTimeout(function () {
                $(waitPopup).show();
                waitPopupSetCookie('wait_popup', '1', 7);
            }, 30000);
        } else {
            window.addEventListener('mouseout', function (e) {
                if (e.clientY < 0 && leavingCount == 0 && waitPopupGetCookie('wait_popup') == null) {
                    $(waitPopup).show();
                    leavingCount++;
                    waitPopupSetCookie('wait_popup', '1', 7);
                }
    ...
    $('#close-wait-popup').click(function(){
        $(waitPopup).hide();
    });
...
function waitPopupSetCookie(name, value, days) {
    var d = new Date();
    d.setTime(d.getTime() + 24 * 60 * 60 * 1000 * days);
    document.cookie = name + "=" + value + ";path=/;expires=" + d.toGMTString();
}
function waitPopupGetCookie(name) {
